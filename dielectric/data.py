...
def get_dataset(
    ...
    if load_preprocessed:
        with open("../preprocessed_dielectric_dataset_elec.pkl", 'rb') as f:
            dataset = pk.load(f)
            dat = []
            f_norm=[]
    ...
    with open("../jarvis_diele_piezo.pkl", 'rb') as f:
        dataset = pk.load(f)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        #performing polar decomposition
        Atemp=dataset[i]['atoms']['lattice_mat']
        Atemp = np.array(Atemp)
        UT, PT = polar(Atemp.T)
        U=UT.T
        P=PT.T
        dataset[i]['atoms']['lattice_mat']=P.tolist()
        dataset[i]["rotation_U"]=U.tolist()

        # label rotation
        np_matrix = np.array(dataset[i]['dielectric'])
        np_matrix=UT@np_matrix@U
        dataset[i]['dielectric']=np_matrix
    ...
    with open("../preprocessed_%s_dataset_elec2.pkl"%dataset_name, 'wb') as f:
        pk.dump(dataset, f)
